import { Controller, Get, Post, Body, Patch, Put, Param, Delete, Query, UseGuards, ValidationPipe, HttpCode, HttpStatus } from '@nestjs/common';
...
export class MeetingRoomsController {
  ...
  create(@Body() createMeetingRoomDto: CreateMeetingRoomDto) {
    return this.meetingRoomsService.create(createMeetingRoomDto).then(data => ({ success: true, data }));
  }
  ...
  findAll(@Query(ValidationPipe) query: QueryMeetingRoomDto) {
    return this.meetingRoomsService.findAll(query).then(res => ({ success: true, ...res }));
  }
  ...
  getAvailableRooms(
    ...
    return this.meetingRoomsService.getAvailableRooms(start, end, cap).then(data => ({ success: true, data }));
  }

  @Get('available')
  @ApiOperation({ summary: '获取可用会议室列表（别名）' })
  aliasAvailable(
    @Query('startTime') startTime: string,
    @Query('endTime') endTime: string,
    @Query('capacity') capacity?: string
  ...
  findOne(@Param('id') id: string) {
    return this.meetingRoomsService.findOne(id).then(data => ({ success: true, data }));
  }
  ...
  update(
    ...
    return this.meetingRoomsService.update(id, updateMeetingRoomDto).then(data => ({ success: true, data }));
  }

  @Put(':id')
  @Roles(Role.ADMIN, Role.MANAGER)
  @ApiOperation({ summary: '更新会议室（PUT别名）' })
  @ApiResponse({ status: 200, description: '会议室更新成功' })
  updatePut(
    @Param('id') id: string,
    @Body() updateMeetingRoomDto: UpdateMeetingRoomDto
  ...
  @ApiResponse({ status: 200, description: '会议室删除成功' })
  @ApiResponse({ status: 404, description: '会议室不存在' })
  ...
  updateStatus(
    ...
    return this.meetingRoomsService.updateStatus(id, status as any).then(data => ({ success: true, data }));
  }
